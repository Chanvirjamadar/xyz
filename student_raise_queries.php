...
// Handle Action: Delete Query
if (isset($_GET['delete_query'])) {
    $delId = intval($_GET['delete_query']);
    if ($delId > 0) {
        $conn->query("DELETE FROM query_replies WHERE query_id = $delId");
        $conn->query("DELETE FROM queries WHERE id = $delId AND student_id = '$studentID'");
        header("Location: student_raise_queries.php?status=deleted");
        exit();
    }
}
...
// Handle Form Submission (Raise Query)
$feedbackMsg = "";
$feedbackType = "";

if (isset($_GET['status']) && $_GET['status'] === 'deleted') {
    $feedbackMsg = "Query deleted successfully.";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_query'])) {
    ...
    if (!empty($category) && !empty($subject) && !empty($message)) {
        $insertSql = "INSERT INTO queries (student_id, category, subject, message, status, created_at) 
                      VALUES ('$studentID', '$category', '$subject', '$message', 'pending', NOW())";
        
        if ($conn->query($insertSql)) {
            $feedbackMsg = "Query raised successfully! Staff will respond shortly.";
            $feedbackType = "success";
        } else {
            $feedbackMsg = "Error submitting query: " . $conn->error;
            $feedbackType = "danger";
        }
    } else {
        $feedbackMsg = "Please fill in all required fields.";
        $feedbackType = "warning";
    }
}

// Fetch all queries raised by this specific student along with faculty replies
$sql = "SELECT * FROM queries WHERE student_id = '$studentID' ORDER BY created_at DESC";
...
<head>
    ...
    <title>Ask Queries | ZEALHUB</title>
    ...
    <style>
        ...
        * {
        ...
        .form-card {
            background: var(--card-bg); padding: 25px; border-radius: 20px; border: 1px solid var(--border); height: fit-content;
        }

        .form-group { margin-bottom: 20px; }
        .form-group label { display: block; font-size: 13px; font-weight: 700; color: var(--text-main); margin-bottom: 8px; }
        .form-control {
            width: 100%; padding: 12px 16px; border-radius: 12px; border: 1px solid var(--border); background: var(--input-bg); color: var(--text-main); font-size: 14px; outline: none;
        }
        .form-control:focus { border-color: var(--primary); }
        textarea.form-control { resize: vertical; min-height: 120px; }

        .btn-submit {
            width: 100%; padding: 14px; background: var(--primary); color: white; border: none; border-radius: 12px; font-weight: 800; font-size: 14px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 10px;
        }
        ...
        .query-card {
            background: var(--card-bg); padding: 22px; border-radius: 20px; border: 1px solid var(--border); margin-bottom: 20px; position: relative;
        }
        ...
        .status-answered { background: rgba(16, 185, 129, 0.1); color: #10b981; }

        .reply-box {
            margin-top: 15px; padding: 15px; background: var(--input-bg); border-radius: 12px; border-left: 4px solid var(--primary);
        }

        .btn-delete-query {
            color: var(--danger); background: rgba(239, 68, 68, 0.08); border: 1px solid rgba(239, 68, 68, 0.2); padding: 4px 10px; border-radius: 8px; font-size: 11px; font-weight: 700; text-decoration: none; display: inline-flex; align-items: center; gap: 4px; transition: all 0.2s ease;
        }
...
<body data-theme="light">
    ...
    <main class="main-content" id="mainContent">
        <div class="page-header-flex">
            <div>
                <h1>Ask Queries ❓</h1>
                <p class="sub-text">Submit your academic or administrative questions directly to faculty members.</p>
            </div>
        </div>
        ...
        <div class="query-grid">
            <!-- Submit Form -->
            <div class="form-card">
                <h3 style="font-size: 18px; font-weight: 800; margin-bottom: 20px; color: var(--text-main);">Raise New Query</h3>
                ...
                <form method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="subject" id="subject" class="form-control" placeholder="e.g. Data Structures Unit 2" required>
                    </div>

                    <div class="form-group">
                        <label for="message">Your Question</label>
                        <textarea name="message" id="message" class="form-control" placeholder="Describe your doubt in detail..." required></textarea>
                    </div>

                    <button type="submit" name="submit_query" class="btn-submit">
            ...
            <!-- Queries List -->
            <div>
                <h3 style="font-size: 18px; font-weight: 800; margin-bottom: 20px; color: var(--text-main);">My Raised Queries</h3>
                ...
                <?php if (!empty($myQueries)): ?>
                        ...
                        <div class="query-card">
                            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px;">
                                ...
                                <div style="display: flex; gap: 8px; align-items: center;">
                                    <span class="status-pill status-<?= ($q['status'] === 'answered' || count($q['replies']) > 0) ? 'answered' : 'pending' ?>">
                                        <?= (count($q['replies']) > 0 || $q['status'] === 'answered') ? 'Answered' : 'Pending' ?>
                                    </span>
                                    <a href="student_raise_queries.php?delete_query=<?= $q['id'] ?>" onclick="return confirm('Are you sure you want to delete this query?');" class="btn-delete-query" title="Delete Query">
                                        <i class="fa-solid fa-trash"></i> Delete
                                    </a>
                                </div>
                            </div>

                            <h4 style="font-size: 15px; font-weight: 800; margin-bottom: 6px; color: var(--text-main);"><?= htmlspecialchars($q['subject']) ?></h4>
                            <p style="font-size: 13.5px; color: var(--text-muted); line-height: 1.5; margin-bottom: 10px;"><?= nl2br(htmlspecialchars($q['message'])) ?></p>
                            <small style="color: var(--text-muted); font-size: 11px;"><i class="fa-regular fa-clock"></i> <?= date('M d, Y h:i A', strtotime($q['created_at'])) ?></small>
                                    ...
                                    <div class="reply-box">
                                        <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                                            <strong style="font-size: 12.5px; color: var(--text-main);">
                                                <i class="fa-solid fa-reply" style="color: var(--primary); margin-right: 5px;"></i>
                                                <?= htmlspecialchars($reply['staff_name']) ?> (Faculty)
                                            </strong>
                                            <small style="font-size: 10px; color: var(--text-muted);"><?= date('M d, H:i', strtotime($reply['replied_at'])) ?></small>
                                        </div>
                                        <p style="font-size: 13px; color: var(--text-main); margin: 0;"><?= nl2br(htmlspecialchars($reply['reply'] ?? ($reply['reply_text'] ?? 'No reply text'))) ?></p>
                                    </div>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
                    <div style="background: var(--card-bg); padding: 40px; text-align: center; border-radius: 20px; border: 1px solid var(--border); color: var(--text-muted);">
                        <i class="fa-solid fa-comments" style="font-size: 35px; opacity: 0.3; margin-bottom: 12px; display: block;"></i>
                        No queries submitted yet. Use the form to submit your first query!
                    </div>
                <?php endif; ?>
            </div>
        </div>
