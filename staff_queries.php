...
if (isset($_GET['delete_id'])) {
    ...
    if ($delId > 0) {
        $conn->query("DELETE FROM query_replies WHERE query_id = $delId");
        $conn->query("DELETE FROM queries WHERE id = $delId");
        header("Location: staff_queries.php?status=deleted");
        exit();
    }
}

// Direct PHP Reply Processing
$replySuccessMsg = "";
$replyErrorMsg = "";

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_reply'])) {
    $query_id = isset($_POST['query_id']) ? intval($_POST['query_id']) : 0;
    $reply_text = isset($_POST['reply_message']) ? mysqli_real_escape_string($conn, trim($_POST['reply_message'])) : '';
    $staff_id_escaped = mysqli_real_escape_string($conn, $staffID);

    if ($query_id > 0 && !empty($reply_text)) {
        // 1. Insert into query_replies
        $insertSql = "INSERT INTO query_replies (query_id, staff_id, reply, replied_at) 
                      VALUES ($query_id, '$staff_id_escaped', '$reply_text', NOW())";
        
        if ($conn->query($insertSql)) {
            // 2. Update status in queries table to answered
            $conn->query("UPDATE queries SET status = 'answered' WHERE id = $query_id");
            
            // Redirect to prevent duplicate submission on page refresh
            header("Location: staff_queries.php?status=success");
            exit();
        } else {
            $replyErrorMsg = "Failed to save reply: " . $conn->error;
        }
    } else {
        $replyErrorMsg = "Please enter a valid response before submitting.";
    }
}

// Fetch queries with LEFT JOIN
$sql = "SELECT q.*, 
        COALESCE(s.name, q.student_id) AS student_name, 
        COALESCE(s.email, 'No email') AS student_email 
        FROM queries q
...
if ($result) {
    while ($row = $result->fetch_assoc()) {
        ...
        $repliesRes = $conn->query("SELECT qr.*, COALESCE(sp.name, qr.staff_id) AS staff_name 
                                    FROM query_replies qr 
                                    LEFT JOIN staff_profile sp ON (qr.staff_id = sp.staff_id OR qr.staff_id = sp.id)
...
<head>
    ...
    <title>Student Queries | ZEALHUB</title>
...
<body data-theme="light">
        ...
        <div class="header-right">
            ...
            <a href="staff_queries.php" class="icon-btn" title="Student Queries">
                ...
                <?php if ($pendingCount > 0): ?>
                    <span class="badge" id="queryBadge"><?= $pendingCount ?></span>
                <?php endif; ?>
            </a>
    ...
    <aside class="sidebar expanded" id="sidebar">
        ...
        <a href="staff_queries.php" class="active"><i class="fa-solid fa-clipboard-question"></i> <span>Student Queries</span></a>
    ...
    <main class="main-content pushed" id="mainContent">
        <div style="margin-bottom: 30px;">
            <h1 style="font-size: 28px; font-weight: 800;">Student Queries</h1>
            <p style="color: var(--text-muted);">Resolve student doubts and academic inquiries in real-time.</p>
        </div>

        <?php if (isset($_GET['status']) && $_GET['status'] === 'success'): ?>
            <div style="padding: 12px 20px; background: rgba(16, 185, 129, 0.15); color: var(--success); border: 1px solid var(--success); border-radius: 12px; margin-bottom: 20px;">
                <i class="fa-solid fa-circle-check"></i> Reply submitted successfully!
            </div>
        <?php endif; ?>

        <?php if (!empty($replyErrorMsg)): ?>
            <div style="padding: 12px 20px; background: rgba(239, 68, 68, 0.15); color: var(--danger); border: 1px solid var(--danger); border-radius: 12px; margin-bottom: 20px;">
                <i class="fa-solid fa-circle-exclamation"></i> <?= htmlspecialchars($replyErrorMsg) ?>
            </div>
        <?php endif; ?>
        ...
        <div class="card">
            ...
            <div id="queriesContainer">
                <?php if (!empty($queriesList)): foreach ($queriesList as $q): ?>
                        <div class="query-card" data-category="<?= strtolower($q['category']) ?>" data-status="<?= strtolower($q['status']) ?>">
                            <div style="display:flex; justify-content:space-between; margin-bottom:12px;">
                                <span class="category-tag"><?= strtoupper(htmlspecialchars($q['category'])) ?></span>
                                <span class="status-badge status-<?= strtolower(htmlspecialchars($q['status'])) ?>"><?= htmlspecialchars($q['status']) ?></span>
                            </div>
                            <h4 style="margin-bottom:5px; color:var(--text-main);"><?= htmlspecialchars($q['subject']) ?></h4>
                            <p style="font-size:13px; color:var(--text-muted); margin-bottom:15px;"><?= nl2br(htmlspecialchars($q['message'])) ?></p>
                            
                            <?php if (!empty($q['replies'])): ?>
                                <div style="margin-bottom: 15px; padding: 12px; background: rgba(255,255,255,0.03); border-radius: 10px; border-left: 3px solid var(--primary);">
                                    <strong style="font-size: 12px; color: var(--primary);">Replies:</strong>
                                    <?php foreach ($q['replies'] as $rep): ?>
                                        <p style="font-size: 12px; margin-top: 5px; color: var(--text-main);">
                                            <strong><?= htmlspecialchars($rep['staff_name']) ?>:</strong> <?= htmlspecialchars($rep['reply']) ?>
                                        </p>
                                    <?php endforeach; ?>
                                </div>
                            <?php endif; ?>

                            <div style="display:flex; justify-content:space-between; align-items:center; border-top:1px solid var(--border); padding-top:15px; flex-wrap:wrap; gap:10px;">
                                <div style="font-size:11px; color:var(--text-muted);">
                                    <strong><?= htmlspecialchars($q['student_name']) ?></strong> • <?= date('M d, H:i', strtotime($q['created_at'])) ?>
                                </div>
                                <div style="display: flex; gap: 8px; align-items: center;">
                                    <a href="staff_queries.php?delete_id=<?= $q['id'] ?>" onclick="return confirm('Are you sure you want to delete this query?');" style="color: var(--danger); background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.2); padding: 6px 12px; border-radius: 10px; font-size: 12px; font-weight: 700; text-decoration: none;" title="Delete Query">
                                        <i class="fa-solid fa-trash"></i> Delete
                                    </a>
                                    <button class="btn-reply" onclick='openReplyModal(<?= htmlspecialchars(json_encode($q), ENT_QUOTES, "UTF-8") ?>)'>
                    ...
                    <p style="text-align:center; padding:40px; color:var(--text-muted);">No queries found.</p>
                <?php endif; ?>
            </div>
        </div>
    </main>
    ...
    <div class="modal" id="replyModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3 id="modalSubject">Reply to Query</h3>
                <button type="button" onclick="closeReplyModal()" style="background:none; border:none; font-size:20px; color:var(--text-muted); cursor:pointer;">&times;</button>
            </div>
            <div style="margin-bottom:20px; padding:15px; background:var(--input-bg); border-radius:12px; border-left:4px solid var(--primary);">
                <p id="modalMessage" style="font-size:14px; color:var(--text-main);"></p>
            </div>
            
            <form id="replyForm" action="staff_queries.php" method="POST">
                <input type="hidden" name="submit_reply" value="1">
                <input type="hidden" name="query_id" id="modalQueryId">
                <textarea name="reply_message" id="replyMessage" class="reply-textarea" placeholder="Type your response here..." required></textarea>
                <button type="submit" class="btn-reply" style="width:100%; justify-content:center;">Send Reply</button>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function applyFilters() {
            ...
            document.querySelectorAll('.query-card').forEach(card => {
                ...
                card.style.display = (matchesCat && matchesStatus) ? 'block' : 'none';
            });
        }

        function openReplyModal(q) {
            document.getElementById('modalQueryId').value = q.id;
            document.getElementById('modalSubject').innerText = 'Reply: ' + (q.subject || 'Student Query');
            document.getElementById('modalMessage').innerText = q.message;
