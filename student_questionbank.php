...
if ($search !== '') {
    ...
    $searchQuery = " AND (qb.subject LIKE '%$safeSearch%' OR qb.title LIKE '%$safeSearch%' OR qb.uploaded_by LIKE '%$safeSearch%' OR sp.name LIKE '%$safeSearch%') ";
}

$dataQuery = "SELECT qb.*, COALESCE(sp.name, qb.uploaded_by, 'Faculty') as faculty_name, sp.subject as staff_specialization 
              FROM question_bank qb 
              LEFT JOIN staff_profile sp ON (qb.uploaded_by = sp.staff_id OR qb.uploaded_by = sp.name OR qb.uploaded_by = sp.id) 
              WHERE 1=1 $searchQuery 
              ORDER BY qb.id DESC";

$qb_res = $conn->query($dataQuery);
$totalPapers = ($qb_res) ? $qb_res->num_rows : 0;
?>
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .page-header-flex h1 { font-size: 26px; font-weight: 800; }

        .total-badge {
            background: var(--card-bg); padding: 10px 20px; border-radius: 14px; border: 1px solid var(--border); text-align: center;
        }

        .total-badge p { font-size: 10px; font-weight: 800; color: var(--text-muted); text-transform: uppercase; }
        .total-badge h2 { margin: 0; color: var(--primary); font-size: 1.4rem; font-weight: 800; }

        .search-card {
            background: var(--card-bg); padding: 20px; border-radius: 18px; border: 1px solid var(--border); margin-bottom: 30px;
        }

        .search-form { display: flex; gap: 12px; }

        .search-input {
            flex: 1; padding: 12px 20px; border-radius: 12px; border: 1px solid var(--border); background: var(--input-bg); color: var(--text-main); font-size: 14px; outline: none;
        }

        .btn-search {
            padding: 12px 24px; background: var(--primary); color: white; border: none; border-radius: 12px; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 8px;
        }

        .table-container {
            background: var(--card-bg); border-radius: 20px; border: 1px solid var(--border); overflow: hidden;
        }

        .custom-table { width: 100%; border-collapse: collapse; text-align: left; }
        .custom-table th {
            background: var(--bg); padding: 16px 20px; font-size: 12px; font-weight: 800; color: var(--text-muted); text-transform: uppercase; border-bottom: 1px solid var(--border);
        }

        .custom-table td {
            padding: 18px 20px; border-bottom: 1px solid var(--border); font-size: 14px;
        }

        .subject-badge {
            background: rgba(67, 97, 238, 0.1); color: var(--primary); padding: 4px 10px; border-radius: 8px; font-size: 12px; font-weight: 700;
        }

        .year-badge {
            background: rgba(16, 185, 129, 0.1); color: var(--success); padding: 3px 8px; border-radius: 6px; font-size: 11px; font-weight: 700; margin-left: 6px;
        }

        .btn-view {
            text-decoration: none; padding: 8px 16px; background: var(--primary); color: white; border-radius: 10px; font-size: 12px; font-weight: 700; display: inline-flex; align-items: center; gap: 6px;
        }

        .btn-download-file {
            text-decoration: none; padding: 8px 16px; background: rgba(16, 185, 129, 0.1); color: var(--success); border-radius: 10px; font-size: 12px; font-weight: 700; display: inline-flex; align-items: center; gap: 6px; margin-left: 6px;
        }
        ...
        .footer {
            margin-top: 40px; padding: 20px 0; border-top: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center; color: var(--text-muted); font-size: 13px;
        }
...
<body data-theme="light">
    ...
    <main class="main-content" id="mainContent">
        <div class="page-header-flex">
            <div>
                <h1>Question Bank 📝</h1>
                <p style="color: var(--text-muted); font-size: 14px;">Access previous year examination papers and practice model question sets.</p>
            </div>
        ...
        <div class="search-card">
            <form method="GET" class="search-form">
                <input type="text" name="search" class="search-input" placeholder="Search by paper title, subject, or faculty..." value="<?= htmlspecialchars($search) ?>">
                <button type="submit" class="btn-search"><i class="fa-solid fa-magnifying-glass"></i> Search</button>
                <?php if ($search !== ''): ?>
                    <a href="student_questionbank.php" style="padding: 12px 18px; background: var(--bg); border: 1px solid var(--border); border-radius: 12px; text-decoration: none; color: var(--text-muted); font-weight: 700; font-size: 13px; display: inline-flex; align-items: center; gap: 6px;">Clear</a>
                <?php endif; ?>
            </form>
        </div>
        ...
        <div class="table-container">
            <table class="custom-table">
                ...
                <tbody>
                    <?php if ($qb_res && $qb_res->num_rows > 0): ?>
                        <?php while ($row = $qb_res->fetch_assoc()): ?>
                            <?php 
                            $staffDisplayName = !empty($row['faculty_name']) && $row['faculty_name'] !== 'Faculty' 
                                ? $row['faculty_name'] 
                                : (!empty($row['uploaded_by']) ? $row['uploaded_by'] : 'Faculty Staff');
                            $staffInitials = strtoupper(substr(preg_replace('/\s+/', ' ', trim($staffDisplayName)), 0, 2)) ?: 'FA';
                            
                            $rawFile = !empty($row['file_name']) ? $row['file_name'] : ($row['file_path'] ?? '');
                            $filePath = (strpos($rawFile, 'uploads/') === 0 || strpos($rawFile, 'http') === 0) 
                                ? $rawFile 
                            ...
                            <tr>
                                <td>
                                    <div style="display:flex; align-items:center; gap:12px;">
                                        <div style="width:38px; height:38px; background:rgba(67,97,238,0.12); color:var(--primary); border-radius:50%; display:flex; align-items:center; justify-content:center; font-weight:800; font-size:12px;">
                                            <?= $staffInitials ?>
                                        </div>
                                        <div>
                                            <strong style="font-weight: 700; color: var(--text-main); font-size: 14px;"><?= htmlspecialchars($staffDisplayName) ?></strong>
                                            <p style="font-size: 10px; color: var(--text-muted); font-weight: 700;">FACULTY MEMBER</p>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    ...
                                    <?php if (!empty($row['year'])): ?>
                                        <span class="year-badge"><i class="fa-regular fa-calendar"></i> <?= htmlspecialchars($row['year']) ?></span>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <strong style="display:block; color: var(--text-main); font-size: 14px;"><?= htmlspecialchars($row['title']) ?></strong>
                                    <?php if (!empty($row['description'])): ?>
                                        <small style="color: var(--text-muted); font-size: 11px;"><?= htmlspecialchars($row['description']) ?></small>
                                    <?php endif; ?>
                                </td>
                                <td style="text-align: right; white-space: nowrap;">
                                    <a href="<?= htmlspecialchars($filePath) ?>" class="btn-view" target="_blank">
                                        <i class="fa-solid fa-eye"></i> View
                                    </a>
                                    <a href="<?= htmlspecialchars($filePath) ?>" class="btn-download-file" download>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="4" style="text-align: center; padding: 60px; color: var(--text-muted);">
                                ...
                                <span style="font-size: 1rem; font-weight: 600;">No question papers found.</span><br>
        ...
        <footer class="footer">
            ...
            <div style="display: flex; gap: 15px;">
                ...
                <a href="student_questionbank.php">Question Bank</a>
