...
$searchQuery = "";
if ($search !== '') {
    $searchQuery = " WHERE staff_name LIKE ? OR subject LIKE ? ";
}

// Fetch Records with Pagination
$countQuery = "SELECT COUNT(*) as total FROM syllabus" . $searchQuery;
$stmt3 = $conn->prepare($countQuery);
if ($search !== '') {
    $stmt3->bind_param("ss", $searchParam, $searchParam);
}
$stmt3->execute();
$totalRecords = $stmt3->get_result()->fetch_assoc()['total'] ?? 0;
$totalPages = max(1, ceil($totalRecords / $limit));

$dataQuery = "SELECT * FROM syllabus" . $searchQuery . " ORDER BY id DESC LIMIT ? OFFSET ?";
$stmt4 = $conn->prepare($dataQuery);
if ($search !== '') {
    $stmt4->bind_param("ssii", $searchParam, $searchParam, $limit, $offset);
} else {
    $stmt4->bind_param("ii", $limit, $offset);
}
$stmt4->execute();
$syllabus_res = $stmt4->get_result();

$totalSubjectsQuery = $conn->query("SELECT COUNT(DISTINCT subject) as total FROM syllabus");
$totalSubjects = ($totalSubjectsQuery) ? $totalSubjectsQuery->fetch_assoc()['total'] : 0;
?>
...
<head>
    ...
    <title>Course Syllabus | ZEALHUB</title>
    ...
    <style>
        ...
        * {
        ...
        .stats-badge p { font-size: 10px; font-weight: 800; color: var(--text-muted); text-transform: uppercase; }
        .stats-badge h2 { margin: 0; color: var(--primary); font-size: 1.4rem; font-weight: 800; }
        ...
        .subject-name { font-weight: 700; color: var(--text-main); }

        .faculty-cell { display: flex; align-items: center; gap: 12px; }
        .avatar-circle-sm {
        ...
        .theme-modal {
            position: fixed; inset: 0; background: rgba(0,0,0,0.5); display: none; align-items: center; justify-content: center; z-index: 3000; padding: 20px;
        }
    ...
    <main class="main-content" id="mainContent">
        <div class="page-header-flex">
            <div>
                ...
                <p style="color: var(--text-muted); font-size: 14px;">Download and review your academic curriculum for this semester.</p>
            </div>
        ...
        <div class="search-card">
            <form method="GET" class="search-form">
                <input type="text" name="search" class="search-input" placeholder="Search by subject or faculty name..." value="<?= htmlspecialchars($search) ?>">
                <button type="submit" class="btn-primary"><i class="fa-solid fa-magnifying-glass"></i> Search</button>
                <?php if ($search): ?>
                    <a href="student_syllabus.php" class="btn-clear">Clear</a>
                <?php endif; ?>
            </form>
        </div>
            ...
            <table class="custom-table">
                <thead>
                    <tr>
                        ...
                        <th>Upload Date / Info</th>
                        <th style="text-align: right;">Action</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if ($syllabus_res && $syllabus_res->num_rows > 0): ?>
                        <?php while ($row = $syllabus_res->fetch_assoc()): ?>
                            <tr>
                                <td><span class="subject-name"><?= htmlspecialchars($row['subject']) ?></span></td>
                                <td>
                                    <div class="faculty-cell">
                                        <div class="avatar-circle-sm">
                                            <?= strtoupper(substr($row['staff_name'] ?? 'F', 0, 2)) ?>
                                        </div>
                                        <span><?= htmlspecialchars($row['staff_name'] ?? 'Faculty') ?></span>
                                    </div>
                                </td>
                                <td>
                                    <span style="color: var(--text-muted); font-size: 13px;">
                                        <?= isset($row['upload_date']) ? date('M d, Y', strtotime($row['upload_date'])) : 'Available' ?>
                                    </span>
                                </td>
                                <td style="text-align: right; white-space: nowrap;">
                                    ...
                                    <a href="<?= htmlspecialchars($row['file_path']) ?>" class="btn-download-file" download>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="4" style="text-align: center; padding: 60px; color: var(--text-muted);">
                                <i class="fa-solid fa-book-open" style="font-size: 3rem; opacity: 0.3; display: block; margin-bottom: 15px;"></i>
                                No syllabus records found matching your search.
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
        ...
        <footer class="footer">
            ...
            <div style="display: flex; gap: 15px;">
                ...
                <a href="student_syllabus.php">Syllabus</a>
